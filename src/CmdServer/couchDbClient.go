...
func DeleteNode(info CouchReturnDocument) {
	...
	deleteRes, _ := http.NewRequest("DELETE", fmt.Sprintf("http://openwhisk:openwhisk@%s:5984/status_sync/%s?rev=%s", COUCHDB_HOST, info.Id, info.Rev), nil)
	_, err := httpClient.Do(deleteRes)
	if err != nil {
